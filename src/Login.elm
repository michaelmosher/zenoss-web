...
hostnameLoginField hostname =
    let
        defaultValue = case hostname of
            "" -> (placeholder "Zenoss Hostname")
            _ -> (value hostname)
    in  
...
usernameLoginField username =
    let
        defaultValue = case username of
            "" -> (placeholder "Zenoss Username")
            _ -> (value username)
    in  
...
passwordLoginField password =
    let
        defaultValue = case password of
            "" -> (placeholder "Zenoss Password")
            _ -> (value password)
    in  
